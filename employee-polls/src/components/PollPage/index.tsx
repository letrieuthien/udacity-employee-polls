        ...
        <Grid container spacing={2} sx={{ marginTop: "1rem" }}>
          <Grid item xs={12} sx={styles}>
            <Typography variant="h5" component="h5">
              <div>Poll by {user?.name}</div>
            </Typography>
          </Grid>
          <Grid item xs={12} sx={styles}>
            <img src={'/src/assets/'+ user?.avatarURL} alt={user?.name} style={{ width: 50, height: 50, marginRight: "10px" }}/>
            ...
            >
              ...
              {hasVoted && (
                <div>
                    <span>
                      {question.optionOne.votes &&
                        question.optionOne.votes.length} Vote
                    </span>
                    <span style={{marginLeft: "8px"}}>({percentageOptionOne})</span>
                </div>
              )}
            </Button>
            <Button
            ...
            >
              ...
              {hasVoted && (
                <div>
                    <span>
                      {question.optionTwo.votes &&
                        question.optionTwo.votes.length} Vote
                    </span>
                  <span style={{marginLeft: "8px"}}>({percentageOptionTwo})</span>
                </div>
              )}
            </Button>
          </Grid>
